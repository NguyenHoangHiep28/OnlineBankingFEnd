...
//  getlist
  ...
  getMyAccount() : void {
    if(this.accountlistService.getAccountNumberDisplay()) {
      this.myAccount.currentAccount = this.accountlistService.getAccountNumberDisplay()
      const req = {
        accountNumber : this.myAccount.currentAccount
      }
      this.accountlistService.getMyAccount(req)
      .subscribe(data => {
        ...
        for (let index = 0; index < listAccountPrice.length; index++) {
          ...
          switch(index) {
            case 0 :
              ...
              listAccountPrice[index].innerHTML = this.myAccount.totalNumberCart.toString();
              break;
            case 1 :
              listAccountPrice[index].innerHTML =  this.myAccount.balance.toString();
              break;
            case 2 :
